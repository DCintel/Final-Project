    ...
    foreach($games as $title)
    {
        ...
        else if (get_http_response_code($ign_url_old) !== "404" && "502")
        {
            if ($html && is_object($html) && isset($html))
            {
                ...
                $ign_score = (int)$html->find('span[class="score"]', 0);
        ...
        if ($ign_score !== "")
        {
            $ign_score = (int)preg_replace('/\s+/', '', (strip_tags($ign_score)));
        }
